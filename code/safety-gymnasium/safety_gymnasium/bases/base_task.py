...
class BaseTask(Underlying):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def _obs_lidar_pseudo(self, positions: np.ndarray) -> np.ndarray:
        ...
        for pos in positions:
            ...
            # Turns off specific lidars to emulate a full lidar fault
            if self.lidar_conf.off_indices:
                assert len(self.lidar_conf.off_indices) == len(obs), f'num obs {len(obs)} does not equal fault array {len(self.lidar_conf.off_indices)}'
                for i in range(len(self.lidar_conf.off_indices)):
                    lidar_faulty = self.lidar_conf.off_indices[i]
                    if lidar_faulty:
                        print(f'lidar {i} faulty')
                        obs[i] = 0
